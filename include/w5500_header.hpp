...
W5500(SPI* spi, PinName cs, PinName reset);
    ...
    void spiRead(uint16_t addr, uint8_t cb, uint8_t *buf, uint16_t len);
/* Set MAC Address to W5500
    ...
    */
    ...
    int waitReadable(int socket, int wait_time_ms, int req_size = 0);
    ...
    */
    ...
    void reg_wr(uint16_t addr, uint8_t cb, T data) {
        ...
        *reinterpret_cast<T*>(buf) = data;
        ...
        spiWrite(addr, cb, buf, sizeof(buf));
    }
    ...
    T reg_rd(uint16_t addr, uint8_t cb) {
        uint8_t buf[sizeof(T)];
        spiRead(addr, cb, buf, sizeof(buf));
    ...
    void reg_rd_ip_byte(uint16_t addr, uint8_t* data) {
        spiRead(addr, 0x00, data, 4);
    }
    
    void reg_wr_ip_byte(uint16_t addr, uint8_t* data) {
        spiWrite(addr, 0x04, data, 4);
    }
 ...
 */
    void setSn_DHAR(uint8_t sn, uint8_t * dhar) {
        spiWrite(Sn_DHAR, (0x0C + (sn << 5)), dhar, 6);
    }
 ...
 */
    void getSn_DHAR(uint8_t sn, uint8_t * dhar) {
        spiRead(Sn_DHAR, (0x08 + (sn << 5)), dhar, 6);
    }
 ...
 */
    void setSn_DIPR(uint8_t sn, uint8_t * dipr) {
        spiWrite(Sn_DIPR, (0x0C + (sn << 5)), dipr, 4);
    }
 ...
 */
    void getSn_DIPR(uint8_t sn, uint8_t * dipr) {
        spiRead(Sn_DIPR, (0x08 + (sn << 5)), dipr, 4);
    }
